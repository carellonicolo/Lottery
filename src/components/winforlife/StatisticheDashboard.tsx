...
import { type ExtractionResult, type MatchResult } from '@/lib/winforlife/winforlife';
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-secondary/50 text-secondary-foreground text-[10px] uppercase font-bold">
              <tr>
                ...
                <th className="px-3 py-2">Numeri Vincenti</th>
                <th className="px-3 py-2 text-center">Numerone</th>
            ...
            <tbody className="divide-y divide-border bg-card">
              {[...history].reverse().slice(0, 10).map((game) => (
                <tr key={game.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-3 py-2 font-mono text-xs font-bold text-primary tracking-tight">
                    {game.extraction.numbers.join(', ')}
                  </td>
                  <td className="px-3 py-2 font-mono text-xs font-bold text-accent text-center">
                    {game.extraction.numerone}
                  </td>
